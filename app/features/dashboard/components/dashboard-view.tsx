  ...
  return (
    <DashboardLayout userEmail={userEmail}>
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-border pb-6 gap-4">
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Resumen General</h1>
          <Button variant="outline" onClick={togglePrivacy} className="w-full sm:w-auto rounded-xl border-border bg-background text-foreground hover:bg-muted">
              ...
              <Card className="overflow-hidden border-border shadow-sm relative">
                ...
                <CardContent className="p-6 sm:p-8">
                  <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between relative z-10">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground mb-1.5 flex items-center gap-2">
                        Patrimonio Neto 
                        <span title="Liquidez Total - Deuda Total = Tu riqueza real" className="flex">
                          <Icon icon="ph:info" className="size-4 text-muted-foreground" />
                        </span>
                      </p>
                      <h2 className={`text-4xl sm:text-5xl font-bold tracking-tight ${netWorth < 0 ? 'text-red-600' : 'text-foreground'}`}>
                        <PrivacyBlur>{formatMoney(netWorth, currency)}</PrivacyBlur>
                      </h2>
                    </div>

                    <div className="flex flex-wrap items-center gap-x-6 gap-y-4 rounded-2xl bg-muted/50 p-4 border border-border">
                      <div>
                        <p className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 uppercase tracking-wider mb-1"><Icon icon="ph:wallet-duotone" className="size-4 text-emerald-500" /> Liquidez Total</p>
                        <p className="text-xl font-semibold text-foreground"><PrivacyBlur>{formatMoney(assets, currency)}</PrivacyBlur></p>
                      </div>
                      <div className="w-px h-8 bg-border hidden sm:block"></div>
                      <div>
                        <p className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 uppercase tracking-wider mb-1"><Icon icon="ph:credit-card-duotone" className="size-4 text-red-500" /> Deuda Total</p>
                        <p className="text-xl font-semibold text-foreground"><PrivacyBlur>{formatMoney(liabilities, currency)}</PrivacyBlur></p>
                      </div>
                    </div>
                  </div>

                  <div className="mt-6 flex h-2.5 w-full overflow-hidden rounded-full bg-secondary relative z-10" title="Proporción Activos vs Pasivos">
        ...
        <div className="space-y-4 pt-4 border-t border-border">
          <h2 className="text-lg font-semibold tracking-tight text-foreground">Tus Cuentas</h2>
              ...
              <Card key={w.id} className="hover:border-primary/30 transition-colors">
                <CardContent className="p-5">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold text-foreground line-clamp-1">{w.name}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{w.is_liability ? 'Pasivo / Deuda' : 'Activo / Liquidez'}</p>
                    </div>
        ...
        <div className="pt-6 border-t border-border">
          <Button 
            ...
            className="w-full sm:w-auto flex items-center gap-2 rounded-xl border-border bg-background text-foreground hover:bg-muted"
          >
          ...
          {showTransactions && (
            <Card className="mt-4 animate-in fade-in slide-in-from-top-2 border-border">
              ...
              <CardContent className="p-0">
                {transactions.length === 0 ? (
                  <div className="p-6 text-center text-sm text-muted-foreground">No hay movimientos recientes registrados en ninguna de tus cuentas.</div>
                ) : (
                  <ul className="divide-y divide-border">
                    {transactions.map(tx => (
                      <li key={tx.id} className="flex flex-col gap-2 p-4 hover:bg-muted/50 transition-colors sm:flex-row sm:items-center sm:justify-between">
                        <div>
                          ...
                          <div className="flex items-center flex-wrap gap-2 mt-1 text-xs text-muted-foreground">
                            ...
                            {tx.wallets?.name && (
                              <>
                                ...
                                <span className="flex items-center font-medium text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10 dark:text-indigo-400 px-2 py-0.5 rounded-md">
